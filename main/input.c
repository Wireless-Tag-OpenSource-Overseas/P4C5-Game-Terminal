...
const uint16_t key_conv_tab[]={
	[KEY_TAB]=K_TAB, [KEY_ENTER]=K_ENTER, [KEY_ESC]=K_ESCAPE, [KEY_SPACE]=K_SPACE,
	[KEY_BACKSPACE]=K_BACKSPACE, [KEY_UP]=K_UPARROW, [KEY_DOWN]=K_DOWNARROW,
	[KEY_LEFT]=K_LEFTARROW, [KEY_RIGHT]=K_RIGHTARROW,
	[KEY_LEFTALT]=K_ALT, [KEY_RIGHTALT]=K_ALT,
	[KEY_LEFTCTRL]=K_CTRL, [KEY_RIGHTCTRL]=K_CTRL,
	[KEY_LEFTSHIFT]=K_SHIFT, [KEY_RIGHTSHIFT]=K_SHIFT,
	...
	[KEY_INSERT]=K_INS, [KEY_DELETE]=K_DEL,
	[KEY_PAGEDOWN]=K_PGDN, [KEY_PAGEUP]=K_PGUP,
...
int QG_GetKey(int *down, int *key) {
	...
	*down=0;
	hid_ev_t ev;
	if (!usb_hid_receive_hid_event(&ev)) return 0;
	if (ev.type==HIDEV_EVENT_KEY_DOWN || ev.type==HIDEV_EVENT_KEY_UP) {
		*down=(ev.type==HIDEV_EVENT_KEY_DOWN)?1:0;
		...
		if (ev.no>2) return 0; //prevent overflow into K_JOY range
		*key=K_MOUSE1+ev.no;
		*down=(ev.type==HIDEV_EVENT_MOUSE_BUTTONDOWN)?1:0;
		return 1;
	} else if (ev.type==HIDEV_EVENT_MOUSE_MOTION) {
		mouse_dx+=ev.mouse_motion.dx/8;
		//mouse_dy disabled: combo receiver sends phantom data on Y axis
